...
- **Landing Page Hero Redesign** — two-column hero layout: left column has a badge, headline, feature checklist, primary/secondary CTAs ("Enhance My Resume" and "See Example"), and social proof; the "Enhance My Resume" CTA routes by auth state — both guests (anonymous sessions) and signed-in users go to `/dashboard`, and unauthenticated users see the auth modal — there is no drag-and-drop upload zone on the home page; right column shows a full resume mock card with Summary, Experience (with bullet points), and Education sections; a trust bar with company logos is rendered as a distinct white rounded card below the hero; background is a light periwinkle (#EEF2FF); expandable feature chips removed
